                ...
                <div v-if="group == 'music'" class="w-full flex justify-center items-center mt-14 animate__animated animate__fadeInUp animate__faster">
                    <swiper wrapperTag="div" tag="div" :pagination="pagination" :modules="modules" class="mySwiper flex flex-wrap justify-center items-center gap-x-4 gap-y-8 2xl:w-[72rem]">
                        <swiper-slide class="flex flex-wrap justify-start items-start gap-x-4 gap-y-8 w-[48rem] pb-20" v-for="musicians in musiciansArray" :key="musicians.id">
                            <div class="w-44 overflow-x-hidden flex flex-col space-y-1 cursor-pointer hover:shadow-md" v-for="item in musicians[0]" :key="item.id">
                                <img class="mx-auto pt-2 w-40 h-44 object-cover grayscale hover:grayscale-0" :src="item.image" :alt="item.name + ' profile photo'">
                                <div class="w-full text-center text-text-base font-[900] text-gray-900 truncate"><span>{{ item.name }} {{ item.surname }}</span></div>
                                <div class="w-full text-center text-md font-[900] text-gray-400"><span>{{ item.category }}</span></div>
                            </div>
                        </swiper-slide>
                    </swiper>
                </div>
                ...
                <div v-if="group == 'dance'" class="w-full flex justify-center items-center mt-14 animate__animated animate__fadeInUp animate__faster">
                    <swiper wrapperTag="div" tag="div" :pagination="pagination" :modules="modules" class="mySwiper flex flex-wrap justify-center items-center gap-x-4 gap-y-8 2xl:w-[72rem]">
                        <swiper-slide class="flex flex-wrap justify-start items-start gap-x-4 gap-y-8 w-[48rem] pb-20" v-for="dancers in dancersArray" :key="dancers.id">
                            <div class="w-44 overflow-x-hidden flex flex-col space-y-1 cursor-pointer hover:shadow-md" v-for="item in dancers[0]" :key="item.id">
                                <img class="mx-auto pt-2 w-40 h-44 object-cover grayscale hover:grayscale-0" :src="item.image" :alt="item.name + ' profile photo'">
                                <div class="w-full text-center text-text-base font-[900] text-gray-900 truncate"><span>{{ item.name }} {{ item.surname }}</span></div>
                                <div class="w-full text-center text-md font-[900] text-gray-400"><span>{{ item.category }}</span></div>
                            </div>
...
export default defineComponent ({
    ...
    setup(props) {
        ...
        const musicians = teams.filter(event => event.group == 'music');

        const dancers = teams.filter(event => event.group == 'dance');

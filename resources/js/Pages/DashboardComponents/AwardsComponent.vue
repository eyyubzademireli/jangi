        ...
        <!-- Info starts -->
            ...
            <div class="w-full flex flex-col space-y-6 mt-10" v-if="view == 'table'">
                ...
                <div v-if="(teams.length == 0 && searchText == '') || (searchResult.length == 0 && searchText != '')" class="w-full h-48 flex justify-center items-center shadow-md bg-gray-100 rounded text-md font-semibold text-gray-700">
                    <span>Heç bir mükafat tapılmadı</span>
                </div>
                <div v-if="(teams.length != 0 && searchText == '') || (searchResult.length != 0 && searchText != '')" class="w-full h-12 flex justify-start items-center px-4 shadow-md bg-gray-100 rounded text-md font-semibold text-gray-700">
                    ...
                    <div class="w-full"><span>Başlıq:</span></div>
                    <div class="w-full"><span>Açıqlama:</span></div>
                ...
                <div class="w-full flex flex-col space-y-2"  v-if="searchText == ''">
                    <div v-for="member in paginatedData" :key="member.id" class="w-full h-12 flex justify-start items-center px-4 shadow-md bg-gray-100 rounded text-md font-semibold text-gray-700">
                        ...
                        <div class="w-full truncate"><span>{{ member.title }}</span></div>
                        <div class="w-full truncate"><span>{{ member.description }}</span></div>
            ...
            <div class="w-full flex flex-col space-y-6 mt-10" v-if="view == 'add'">
                ...
                <div class="w-full flex flex-col space-y-4">
                    <input class="rounded w-full" v-model="title" type="text" placeholder="Başlıq">
                    <textarea v-model="description" class="rounded w-full" name="description" id="" cols="30" rows="10"></textarea>
                </div>
...
export default defineComponent ({
    ...
    setup(props) {
        ...
        const dialog = {
            title: 'Mükafatın ləğv edilməsi',
            text: 'Mükafatı ləğv etmək istədiyinizə əminsiniz?',
        }
        let alert = ref({
            title: '',
            text: '',
        ...
        const search = () => {
            if(searchText.value == '') {
                ...
                searchResult.value = teams.value.filter(team => {
                    if (team.title.toLowerCase().includes(searchText.value.toLowerCase())) {
                        return team;
                    }
                });
            }
        ...
        const remove = () => {
            axios.post('/composer/awards/remove', {
            ...
            .then(function (response) {
                if(response.data.success == true) {
                    ...
                    showAlert('Təbriklər', 'Mükafat uğurla ləğv edildi');
                    teams.value = response.data.teams;
                    paginate();
                }else {
                    dialogShow.value = false;
                    currentMember.value = null;
                    showAlert('Xəta', 'Mükafatı ləğv etmək mümkün olmadı');
                }
            })
        ...
        const create = () => {
            axios.post('/composer/awards/add', {
                title: title.value,
                description: description.value,
                image: imageLink.value,
            }).then(function (response) {
                if(response.data.success == true) {
                    showAlert('Təbriklər', 'Yeni mükafat uğurla əlavə edildi');
                    teams.value = response.data.teams;
                    paginate();
                    view.value = 'table';
                }else {
                    showAlert('Xəta', 'Mükafat əlavə edilə bilmədi');
                }
            }).catch(function (error) {
                showAlert('Xəta', 'Mükafat əlavə edilə bilmədi');
            })
        ...
        const update = () => {
            disabled.value = true;
            axios.post('/composer/awards/update', {
                ...
                if(response.data.success == true) {
                    showAlert('Təbriklər', 'Mükafat uğurla redaktə edildi');
                    teams.value = response.data.teams;
                    paginate();
                    view.value = 'table';
                }else {
                    showAlert('Xəta', 'Mükafat redaktə edilə bilmədi');
                }

    ...
    <div class="w-full h-full flex flex-col min-h-[560px] select-none px-2">
        ...
        <!-- Info starts -->
            ...
            <div class="w-full flex flex-col space-y-6 mt-10" v-if="view == 'table'">
                <div class="w-full h-12 flex justify-end items-center">
                ...
                <div v-if="teams.length == 0" class="w-full h-48 flex justify-center items-center shadow-md bg-gray-100 rounded text-md font-semibold text-gray-700">
                    <span>Heç bir şəkil tapılmadı</span>
                </div>
                <div v-if="teams.length != 0" class="w-full h-12 flex justify-start items-center px-4 shadow-md bg-gray-100 rounded text-md font-semibold text-gray-700">
            ...
            <div class="w-full flex flex-col space-y-6 mt-10" v-if="view == 'edit'">
                <div class="w-full flex space-x-6 items-start">
                    <div class="w-48">
                        <img :src="imageLink" class="w-44 h-44 rounded object-cover" alt="">
                    </div>
...
export default defineComponent ({
    ...
    setup(props) {
        ...
        const dialog = {
            title: 'Şəklin ləğv edilməsi',
            text: 'Şəkli ləğv etmək istədiyinizə əminsiniz?',
        }
        let alert = ref({
            title: '',
            text: '',
        ...
        const remove = () => {
            axios.post('/composer/media/remove', {
            ...
            .then(function (response) {
                if(response.data.success == true) {
                    ...
                    showAlert('Təbriklər', 'Şəkil uğurla ləğv edildi');
                    teams.value = response.data.teams;
                    paginate();
                }else {
                    dialogShow.value = false;
                    currentMember.value = null;
                    showAlert('Xəta', 'Şəkli ləğv etmək mümkün olmadı');
                }
            })
        ...
        const create = () => {
            axios.post('/composer/media/add', {
                image: imageLink.value,
                group: group.value,
            }).then(function (response) {
                if(response.data.success == true) {
                    showAlert('Təbriklər', 'Yeni şəkil uğurla əlavə edildi');
                    teams.value = response.data.teams;
                    paginate();
                    view.value = 'table';
                }else {
                    showAlert('Xəta', 'Şəkil əlavə edilə bilmədi');
                }
            }).catch(function (error) {
                showAlert('Xəta', 'Şəkil əlavə edilə bilmədi');
            })
        ...
        const update = () => {
            disabled.value = true;
            axios.post('/composer/media/update', {
                id: currentMemberEdit.value.id,
                image: imageLink.value,
                group: group.value,
            }).then(function (response) {
                if(response.data.success == true) {
                    showAlert('Təbriklər', 'Şəkil uğurla redaktə edildi');
                    teams.value = response.data.teams;
                    paginate();
                    view.value = 'table';
                }else {
                    showAlert('Xəta', 'Şəkil redaktə edilə bilmədi');
                }
